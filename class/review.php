<?php 
    ...
    class review{
        ...
        public static function getByIDProduct($id){
            $sql = "SELECT * FROM review WHERE ProductID = '$id' ORDER BY Time DESC";
            return Select($sql);
        }
        ...
        public static function avgRating($id){
            $sql = "SELECT SUM(Rating)/COUNT(CustomerID) FROM review WHERE ProductID = '$id'";
            return Select($sql)->fetch_array()[0];
        }

        public static function sumReview($id){
            $sql = "SELECT COUNT(CustomerID) FROM review WHERE ProductID = '$id'";
            return Select($sql)->fetch_array()[0];
        }

        public static function getNumberStar($id, $star){
            $sql = "SELECT COUNT(CustomerID) FROM review WHERE ProductID = '$id' AND Rating = '$star'";
            return Select($sql)->fetch_array()[0];
